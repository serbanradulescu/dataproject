...
### b3. Anova, Shapiro Wilk, and Kruskal Wallis to study the effect of treatment on behaviour
...
1. For Waiting, Flying, Pruning behaviours treatment did not have any effect on the amount of time spent by the adult hoverfly practicing each behaviour.
...
### c1: Test for normality using the Shapiro–Wilk statistic to determine the normality of the difference in weigth
```{r Test for normality using the Shapiro–Wilk statistic to determine the normality of the difference in weigth, echo=TRUE}

ggdensity(Fordifference$Difference, fill = "lightgray", title = "Visual judgement of the distribution shape of the weight difference")
...
### c2: P.Value for Shapiro.test after the ANOVA
```{r echo=TRUE}
...
## Based on the shapiro.test, we can conclude that the distribution of weight diverge from normal distribution. There is need to use another statistical test which can perfectly analyze a not normal distributed data.

```


### c3: Kruskal-Wallis  analysis and Visual representation of the analysis
```{r echo=TRUE}
