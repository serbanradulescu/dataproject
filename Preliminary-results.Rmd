...
```{r effect of treatments on survival, fig.cap = "Figure 1: Plot to see the differences between blocks and treatments", echo=TRUE}
...
knitr::kable(f1$p.value[,1], caption = "Table that shows pvalue for the mumti-fisher test, each treatment being comaperd with control")
...
```{r effect of treatments on REDEFINED survival,fig.cap = "Figure 2: Plot to see the differences between blocks and treatments on REDEFINED survival", error=FALSE, echo=TRUE}
...
f1 <- fisher.multcomp(t1)
knitr::kable(f1$p.value[,1],cap = "This table shows the multi-comparation fisher test, each treatment compared with the control for REDEFINES survival")
...
```{r Kaplan-Meier curves, fig.cap = "Figure  Kaplan-Meier curves for survival" , error=FALSE, echo=TRUE}
...
### a4. Effect of treatments on time to pupate (ANOVA): Treatment 500 is the only one that shows significant differences #PLOT

```{r Effect of treatments on time to pupate (ANOVA), fig.cap="Figure  Kaplan-Meier curves for time to pupate", error=FALSE, echo=TRUE}
...
```{r HSD Test for weight, fig.cap="Figure  HSD test to see the effect of treatment on weight", error=FALSE, echo=TRUE}
...
```{r, fig.cap = "Figure x: From the Eigen values and the Scree plot of our PCA we understand that Principal Component 1 and Principal Component 2 explain 46.4% of the variance in the data.", error=FALSE}
...
```{r, fig.cap = "Figure x Heat map for behaviour (A heat map is data analysis software that uses color the way a bar graph uses height and width: as a data visualization tool.)", error=FALSE}
...
##### Correlation circle
```{r, fig.cap = "Figure x: This plot of the correlation circle visualizes the correlation between the first two principal components which explain the highest variance which are respectively 27.6 % and 18.8%. We can conclude that Flying and walking are correlated together (the arrow are grouped), stationary and  grooming are correlated together and the arrow which reprents feeding on nectar, pollen, and water is in the opposite direction and not correlated with others behaviours. ", error=FALSE,warning=FALSE}
...
```{r, fig.cap = "Figure x: This plot with ellipses shows the distribution of each behaviour following each treatment. From this plot we can conclude that there is no significant difference between the effect of treatment on different behaviours because the ellipses overlap", error=FALSE,warning=FALSE}
...
###### The p values for stationary, grooming, waiting, flying, probing, feeding, mating respectively.
```{r}
...
###### From the Kruskal Waliis test we can see that
###### 1. For Waiting, Flying, Probing behaviours treatment did not have any effect on the amount of time spent by the adult hoverfly practicing each behaviour.
###### In the rest of the behaviours, the time  spent in each behaviour was different but there is no pattern in their classification. For example, in Mating behaviour, the control adults and the adults exposed to 500 ppb of thiamethoxam are grouped together.

#### b4. Comparing the effect of weight on behaviour
...
G1 <- ggplot(data=WeiBeha2, aes(x= Treatments, y=seconds, fill = Behaviour)) +
  ...
  ggtitle("Effect of weight on behaviour")
...
ggdensity(res.aov$residuals, fill = "lightgray", 
          title = "Distribution shape of the data")
...
#### Fisher's exact test and fisher multcomp test between between FIL and the treatment. Done to determine if there are significant difference between treatments and FIL
```{r echo=TRUE}
ff <- table(Fordifference$FIL, Fordifference$Treatment)
ff1 <- fisher.test(ff)
print (ff1[["p.value"]])

#Based on the fisher's test (i.e p.value = 0.3198), there are no significant difference between the treatment

ff2 <- fisher.multcomp(ff)

knitr::kable( ff2[["p.value"]])
...
The redefine of survival has lead to the same results as the initial survival.
...
All in all, the objective of reproducing the study was reached succesfully and the extra tests and analysis we've done reached the same conclusion.

## 8. Conclusions.
Place for improvement for the authors: 
-impossible to track individuals between data sets (they had other numbers in survival and weight)
-more analysis could be done by the original authors: redefine survival, heat maps or whatever is useful)
-the data sets notation is not clearly explained(a lot of time was spend to understand what FIL means, we are almost sure that it means Found In Leaves)
-including a multivariate analysis like PCA would be a better choice than Kruskal-Wallis for individual behaviour
-they never responded to our questions about the study on the peerj platform

However, we strongly appreciate the author's work:
-the theme is very relevant, supporting sustainability and biodiversity (looking at a pollinator)
-easily overall understandability and simplicity

We also appreciate the complex experimental design: 
-they found a place with no history of insecticide use 
-including eggs were collected from different females to account for genetical variation with respect to thiamethoxin tolerance (in different experience round)
-elaborate measures to simulate natural environment

We have advanced a lot
-github in a team and how to correct it's errors
-teamwork in general
-countless new R and statistical practical and theoretical knowledge: Kaplan-Meier curves, how to transform the data, how to study and analyze categorical data
PCA: elipses, Heatmap , Correlation circle

We practically used what we learned in the klass: ANOVA, Shapiro, Kruskal-Walis,ggplot function, Fisher Test and stat compare mean

We still have a lot to learn:
-syntax to be learned
-a lot of statistical tests, tools and graphical representations to be "discovered" by each one
-how to come up with ideas for our own research
-how to organize our own research and the collected data
-how to deal with larger data sets (from this project it was small)
-because of the poor quality of the data notations between the different data sets(survival, weight, behaviour) we coul not do addition analysis like mediation and moderation and that is something we would have liked to practice
-how to make more visually pleasing Rmarkdown documents